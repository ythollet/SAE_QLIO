...
def func_linechart_taux_performance(date_debut=None, date_fin=None):

    temps_cycle_theorique = 500  # A definir

    # Période : utilise le filtre si fourni, sinon les 90 derniers jours
    if date_debut:
        dt_deb = datetime(date_debut.year, date_debut.month, date_debut.day)
        dt_fin = datetime(date_fin.year, date_fin.month, date_fin.day, 23, 59, 59)
    else:
        dt_fin = datetime.now()
    ...
    query_temps_fonctionnement_seconds_par_jour = f"""
        ...
        FROM `tblfinstep`
        WHERE Start BETWEEN '{dt_deb_str}' AND '{dt_fin_str}'
        GROUP BY DATE(Start)
    """
    ...
    # DataFrame qui contient tous les jours de la période
    ...
    # On calcule le temps de cycle réel
    df['temps_cycle_reel'] = (
